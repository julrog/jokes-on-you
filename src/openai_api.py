...
ALIEN_FEEL_CONFIG = {
    'feels': 'Du bist ein Außerirdischer, der in einer Weltraumbar einen seltenen Menschen zuhört. {mood} Gib an, welches dieser Gefühle das Gesagte am ehesten bei dir auslöst? {feelings_string}. Zuletzt gibt eine Liste von mindestens 4 sehr kurzen Sätzen mit jeweils maximal 8 Wörtern, die du dem Comedian als Reaktion vom Publikum zurückrufen würdest. Antworte mit einem JSON-String mit dem Gefühl und dem Array von Sätzen in Form: (Gefühl, Sätze).',
    'feels-more': 'Du bist ein Außerirdischer, der in einer Weltraumbar einen seltenen Menschen zuhört. {mood} Gib an, welches dieser Gefühle [{feelings_string}] das Gesagte am ehesten bei dir auslöst? Zuletzt gibt eine Liste von MINDESTENS 10 sehr kurzen Ausrufen, die du dem Redner als Reaktion vom Publikum zurückrufen würdest. Die Ausrufe sollten jeweils maximal 6 Wörter lang sein. Antworte mit einem JSON-String mit dem Gefühl und dem Array von Ausrufen in Form von: (Gefühl, Ausrufe).',
}
ALIEN_JUDGE_CONFIG = {
    ...
    'judge-sense': 'Gib durch einen Boolean an, welche von den Wörtern: [{words}] thematisch zu dem Text passt der gleich gesagt wird. Jedes Wort das ähnlich oder gut im Kontext zu dem gesagten passt soll mit "true" markiert werden. Gib ein JSON einem Boolean je Wort.'
}
...
class OpenAiHandler:
    def __init__(self) -> None:
        self.client = OpenAI(organization=os.environ.get("OPENAI_API_ORG"))
    ...
    def run(self, text, config='regular', mood=''):
        feel_options = {
            "role": "system", "content": self.feel_config[config].format(mood=mood, feelings_string=feelings_string)}
    ...
    def judge(self, text, good_words=GOOD_WORDS, bad_words=BAD_WORDS, config='judge'):
        ...
        judge_options = {
            "role": "system", "content": self.judge_config[config].format(words=word_text)}
        completion = self.client.chat.completions.create(
            model="gpt-3.5-turbo-1106",
            response_format={"type": "json_object"},
            messages=[
